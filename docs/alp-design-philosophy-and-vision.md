...
| `model` / `reasoningEffort` | Bắt buộc khai đủ hai phía. Với `main` và `oracle`, **dial công suất** (`low`/`medium`/`high`/`ultra`, §4.1 architecture) ghi đè lúc phóng: cùng một definition chạy bốn model khác nhau, nên `ExecutionPolicy.mode` phải ghi lại nấc đã chạy — không thì `policy.json` mô tả một execution mà nó không mô tả nổi. Nấc gõ sai dừng ngay chứ không rơi về mặc định |
