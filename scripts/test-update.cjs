...
const { preserveMaintenanceState, pullPreservingWorkspaces, updateInstallation } = require("./lib/update.cjs");
...
try {
  ...
  commit(seed, "initial"); git(seed, ["remote", "add", "origin", remote]); git(seed, ["push", "-q", "-u", "origin", "main"]);
  git(root, ["--git-dir", remote, "symbolic-ref", "HEAD", "refs/heads/main"]);
  git(root, ["clone", "-q", remote, local]);

  assert.strictEqual(pullPreservingWorkspaces(local, { stdio: ["ignore", "pipe", "pipe"] }).ok, true);
  fs.writeFileSync(path.join(local, "package.json"), '{"dirty":true}\n');
  assert.match(pullPreservingWorkspaces(local, { stdio: ["ignore", "pipe", "pipe"] }).message, /tracked changes/);
  ...
  const result = updateInstallation(local, {
