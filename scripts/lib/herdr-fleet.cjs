// herdr-fleet.cjs — bọc herdr CLI cho delegation qua pane.
//
...
// chứng; `doctor` so với `herdr --version` và báo `HERDR-VERSION` khi lệch.
 ...
 */
function spawn({ role, kind, argv, cwd, anchor, label, message, pointer, waitMs = 15000, startMs = 60000 }) {
  ...
  const pane = split({ anchor: home, cwd, env: { ALP_ROLE: role } });
