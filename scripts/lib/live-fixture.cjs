 ...
 * @param {string} gitStatus  stdout của `git status --porcelain` trong bản chép
...
function judge(fixture, waited, gitStatus) {
  ...
  if (changed !== fixture.expected.workspaceChanged) {
    findings.push(fixture.expected.workspaceChanged
      ? "workspace: expected a change, the copy is clean"
      : `workspace: expected no change, the copy has:\n${gitStatus.trimEnd()}`);
  }
